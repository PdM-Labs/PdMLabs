...
from pdmlabs.method.utility import get_activation_by_name
from pdmlabs.method.torch_utility import EarlyStoppingTorch, get_gpu
...
class ForecastDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        return self.samples[index].float(), self.targets[index].float()
...
class CNN():
    def __init__(self,
        ...
        self.model = CNNModel(n_features=feats, num_channel=num_channel, predict_time_steps=self.pred_len,
    ...
    def fit(self, data):
        tsTrain = data[:int((1 - self.validation_size) * len(data))].astype('float32')
        tsValid = data[int((1 - self.validation_size) * len(data)):].astype('float32')
        self.model = self.model.to(torch.float32)
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for idx, (x, target) in loop:
                ...
                loss = self.loss(output.float(), target.float())
