...
class XGBoost(SupervisedMethodInterface):
    ...
    def fit(self, historic_data: list[pd.DataFrame], historic_sources: list[str], event_data: pd.DataFrame,
        ...
        for current_historic_data, current_historic_source, labels in zip(historic_data, historic_sources,
                                                                          anomaly_ranges):
            self.model_per_source[current_historic_source] = xgb.XGBClassifier(*self.initial_args,
                                                                               **self.initial_kwargs)
    ...
    def predict(self, target_data: pd.DataFrame, source: str, event_data: pd.DataFrame) -> list[float]:
        ...
        return self.model_per_source[source].predict_proba(target_data)[:, 1].tolist()
    ...
    def get_params(self) -> dict:
        return {
            **(xgb.XGBClassifier(novelty=False, *(self.initial_args), **(self.initial_kwargs)).get_params()),
        }
