...
class LocalOutlierFactorUnsupervised(UnsupervisedMethodInterface):
    def __init__(self, 
    ...
    def _inner_batch_predict(self,dfr:pd.DataFrame):
        tempmodel = self.clf_class(novelty=False, *(self.initial_args), **(self.initial_kwargs))
        final_score = {}
        pos = 0
        df=dfr.values
        for pos in range(self.window, len(df), self.slide):
            ...
            scores = -tempmodel.negative_outlier_factor_
    ...
    def __str__(self) -> str:
        return 'LocalOutlierFactorUnsupervised'


    def get_params(self) -> dict:
        return {
            **(self.clf_class(novelty=False, *(self.initial_args), **(self.initial_kwargs)).get_params()),
            'window': self.window,
            'slide': self.slide,
            'policy': self.policy
        }
