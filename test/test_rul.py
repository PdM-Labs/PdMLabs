...
from pdmlabs.method.xgboostRUL import XGBoostRUL
...
def main():
    ...
    print("Preparing RUL Dataset format...")
    train_data, test_data = dataset_obj.get_rul_dataset()
    
    print("Running Experiment (RUL)...")
    methods = [XGBoostRUL]
    ...
    best_params = run_experiment(
        ...
        method_names=['XGBoostRUL'],
        experiments=[SupervisedRULPdMExperiment],
        experiment_names=['Test_RUL'],
        MAX_RUNS=1,
        MAX_JOBS=1,
        INITIAL_RANDOM=1,
    ...
    experiment_name = 'Test_RUL XGBoostRUL'
    ...
    for target_data, target_source in zip(test_data['target_data'], test_data['target_sources']):
        ...
        print(f"Predictions successfully generated for source {target_source}! Size: {len(preds['rul_predictions'])}")
        
    print("RUL Test Successful.")
